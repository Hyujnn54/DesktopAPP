...
#include "meeting.h"
#include "connection.h"  // Added to fix Connection error
#include <QMessageBox>
#include <QDebug>
    ...
    ,isDarkTheme(false)
{
    ...
    ui->tableWidget->setColumnCount(8);  // Update column count to 8
    ui->tableWidget->setHorizontalHeaderLabels({"ID", "Title", "Organiser", "Participants", "Agenda", "Duration", "Status", "Date"}); // Add "Date" column
...
void MainWindow::handleAddButtonClick() {
    ...
    QDate date = ui->dateEdit->date(); // Get date value
    ...
    meeting m(title, organiser, participant, agenda, duration, date);
    ...
    if (m.add()) {
        ...
        ui->dateEdit->setDate(QDate::currentDate()); // Reset date field
...
void MainWindow::handleDeleteButtonClick() {
    ...
    if (m.delet(id)) {  // Note: 'delet' instead of 'delete' due to your original code
        ...
        QMessageBox::warning(this, "Error", "Failed to delete meeting: " +
                                                Connection::getDatabase().lastError().text());
    }
}
...
void MainWindow::handleUpdateButtonClick() {
    ...
    // Get current meeting data from the selected row
    meeting m;
    m.setId(ui->tableWidget->item(row, 0)->text().toInt());
    ...
    m.setDate(QDate::fromString(ui->tableWidget->item(row, 7)->text(), "yyyy-MM-dd"));

    // Show update dialog
    UpdateDialog dialog(m, this);
    if (dialog.exec() == QDialog::Accepted) {
        meeting updatedMeeting = dialog.getUpdatedMeeting();

        // Validate inputs
        QStringList validationErrors;
        if (updatedMeeting.getTitle().isEmpty())
            validationErrors << "Title cannot be empty";
        if (updatedMeeting.getOrganiser().isEmpty())
            validationErrors << "Organiser cannot be empty";
        if (updatedMeeting.getDuration() <= 0)
            validationErrors << "Duration must be a valid positive number";
        ...
        if (updatedMeeting.update()) {
            QMessageBox::information(this, "Success", "Meeting updated successfully.");
            refreshTableWidget();
        } else {
            QMessageBox::warning(this, "Error", "Failed to update meeting: " +
                                                    Connection::getDatabase().lastError().text());
        }
    }
}
