...
void ClientManager::initialize(Ui::MainWindow *ui)
{
    ...
    // Initialize client date consultations view
    ui->clientDateConsultationsView->setModel(clientTableModel); // Reuse clientTableModel for simplicity
    ...
    // Set initial notification label text
    ...
    connect(ui->trainingNotificationLabel, &ClickableLabel::clicked, this, &ClientManager::on_trainingNotificationLabel_clicked);
...
void ClientManager::on_clientAddButton_clicked()
{
    ...
    if (result.first) {
        ...
        logNotification("Added Client", "Manage Clients Tab", QString("Client: %1").arg(name));
        refreshClientTable();
...
void ClientManager::on_clientDeleteButton_clicked()
{
    ...
    QString clientName = clientProxyModel->data(clientProxyModel->index(selectedRows[0].row(), 0)).toString();
    ...
    if (reply == QMessageBox::Yes) {
        if (client->removeByName(clientName)) {
            ...
            logNotification("Deleted Client", "Manage Clients Tab", QString("Client: %1").arg(clientName));
            refreshClientTable();
            updateCalendarConsultations();
        } else {
...
void ClientManager::on_clientUpdateButton_clicked()
{
    ...
    QString name = clientProxyModel->data(clientProxyModel->index(selectedRows[0].row(), 0)).toString();
    QString sector = clientProxyModel->data(clientProxyModel->index(selectedRows[0].row(), 1)).toString();
    QString contact = clientProxyModel->data(clientProxyModel->index(selectedRows[0].row(), 2)).toString();
    QString email = clientProxyModel->data(clientProxyModel->index(selectedRows[0].row(), 3)).toString();
    QDateTime consultationDate = clientProxyModel->data(clientProxyModel->index(selectedRows[0].row(), 4)).toDateTime();
    QString consultantName = clientProxyModel->data(clientProxyModel->index(selectedRows[0].row(), 5)).toString();
    ...
    if (dialog.exec() == QDialog::Accepted) {
        ...
        if (updateClient(name, newName, newSector, newContact, newEmail, newDateTime, newConsultant)) {
            ...
            logNotification("Updated Client", "Manage Clients Tab", QString("Client: %1 updated to %2").arg(name, newName));
            refreshClientTable();
            updateCalendarConsultations();
        } else {
...
void ClientManager::on_trainingNotificationLabel_clicked()
{
    ...
    QMessageBox::information(qobject_cast<QWidget*>(parent()), "Notifications", notificationText);
}

void ClientManager::logNotification(const QString &action, const QString &location, const QString &details)
{
...
void ClientManager::updateSelectedDateInfo(const QDate &date)
{
    ...
    clientTableModel->setQuery(QString(
    ...
    if (clientTableModel->lastError().isValid()) {
        qDebug() << "Consultation query failed:" << clientTableModel->lastError().text();
        QMessageBox::warning(qobject_cast<QWidget*>(parent()), "Query Error",
                             "Failed to load consultations: " + clientTableModel->lastError().text());
        return;
    }

    ui->clientDateConsultationsView->setModel(clientTableModel);
    ui->clientDateConsultationsView->horizontalHeader()->setSectionResizeMode(QHeaderView::Stretch);
    // Set custom headers
    clientTableModel->setHeaderData(0, Qt::Horizontal, "Client Name");
    clientTableModel->setHeaderData(1, Qt::Horizontal, "Sector");
    clientTableModel->setHeaderData(2, Qt::Horizontal, "Contact Info");
    clientTableModel->setHeaderData(3, Qt::Horizontal, "Consultation Date");
    clientTableModel->setHeaderData(4, Qt::Horizontal, "Consultant");

    // Update consultation count label
    int count = clientTableModel->rowCount();
...
void ClientManager::exportClientsToPdf()
{
    ...
    // Draw rows
    for (int row = 0; row < clientTableModel->rowCount(); ++row) {
        x = 20;
        for (int col = 0; col < headers.size(); ++col) {
            int columnWidth = tableWidth * columnWidths[col];
            QString text = clientTableModel->data(clientTableModel->index(row, col)).toString();
